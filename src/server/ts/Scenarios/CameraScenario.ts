...
export const CameraScenario = (): THREE.Scene => {
	...
	const reelIn = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.1, 32), new THREE.MeshStandardMaterial({ color: Number(Utility.getRandomMutedColor("0x")) }))
	...
	const reelOut = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.1, 32), new THREE.MeshStandardMaterial({ color: Number(Utility.getRandomMutedColor("0x")) }))
